...
## 11. Relação com as Variáveis do Processo

| Etapa | Variáveis relacionadas | Função das variáveis |
| --- | --- | --- |
| **1. Recepção e Alimentação dos Grãos** | `LIT-101` | Medir continuamente o nível do funil de recepção para garantir o abastecimento contínuo e evitar operação a seco ou transbordo. |
| **2. Alimentador Vibratório** | `Comando do Alimentador Vibratório` *(Driver PWM / Inversor)* | Sinal discreto de saída do PLC/Driver para acionar ou desligar a vibração da calha dosadora de grãos. |
| **3. Transporte pela Esteira** | `ST-201`<br>

<br>`JI-201` | **ST-201:** Medir a velocidade real da esteira para cálculo de tempo de trânsito e sincronismo.<br>

<br>**JI-201:** Indicar condição de sobrecarga no motor da esteira para proteção mecânica/elétrica. |
| **4. Pesagem** | `WT-301`<br>

<br>`FT-301` | **WT-301:** Medir a massa física instantânea incidente sobre a balança contínua.<br>

<br>**FT-301:** Indicar a vazão mássica calculada pelo PLC para acompanhamento da taxa de produção em tempo real. |
| **5. Inspeção por Visão Computacional** | `XS-401`<br>

<br>`KSA-401` | **XS-401:** Gerar o pulso de trigger óptico na passagem do grão para disparo da foto.<br>

<br>**KSA-401:** Monitorar a prontidão, comunicação e operação da câmera e sistema de visão. |
| **6. Classificação dos Grãos** | `KXA-501`<br>

<br>`KXA-503` | **KXA-501:** Indicar o resultado lógico de aprovação integral do grão (Categoria A).<br>

<br>**KXA-503:** Indicar o resultado lógico de rejeição do grão por defeito/impureza (Categoria C). |
| **7. Sistema Pneumático e Ejeção** | `PT-601`<br>

<br>`PAL-601`<br>

<br>`FY-603`<br>

<br>`ZSH-601` | **PT-601:** Medir continuamente a pressão de ar da linha pneumática.<br>

<br>**PAL-601:** Sinalizar alarme de pressão insuficiente para a realização segura da ejeção.<br>

<br>**FY-603:** Comandar o disparo da válvula solenoide do ejetor de rejeito.<br>

<br>**ZSH-601:** Confirmar o avanço físico do cilindro/atuador pneumático de ejeção. |
| **8. Coleta dos Produtos** | `LIT-703` | Medir continuamente o nível de preenchimento do recipiente de coleta de rejeitos para alerta de substituição. |
| **9. Supervisão pelo SCADA** | `Permissão Geral de Operação`<br>

<br>`Taxa de Rejeição Total` | **Permissão Geral de Operação:** Validar o intertravamento global de segurança do PLC para liberação da operação.<br>

<br>**Taxa de Rejeição Total:** Apresentar no SCADA o indicador percentual acumulado (KPI) de grãos rejeitados no lote. |
